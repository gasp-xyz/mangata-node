...
pub mod phragmen_elections {
	...
	use frame_support::{migration::{storage_key_iter, storage_iter, have_storage_value}, Twox64Concat};
	...
	impl OnRuntimeUpgrade for PhragmenElectionsMigration {
		...
		fn pre_upgrade() -> Result<(), &'static str> {
            ...
            assert!(storage_key_iter::<<Runtime as frame_system::Config>::AccountId,deprecated::Voter<<Runtime as frame_system::Config>::AccountId, Balance>,Twox64Concat>(
                b"Elections",
                b"Voting",
            ).next().is_some());
