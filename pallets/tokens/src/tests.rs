...
fn transfer_all_should_work() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            let transferred_event =
                TestEvent::tokens(RawEvent::Transferred(TEST_TOKEN_ID, ALICE, BOB, 100));
            assert!(System::events()
                .iter()
                .any(|record| record.event == transferred_event));
        });
...
fn currency_adapter_ensure_currency_adapter_should_work() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_eq!(
                Tokens::reserved_balance(TEST_TOKEN_ID, &TREASURY_ACCOUNT),
                5
            );
            assert_eq!(Tokens::free_balance(TEST_TOKEN_ID, &TREASURY_ACCOUNT), 95);
            assert_eq!(
                <Runtime as pallet_elections_phragmen::Trait>::Currency::total_balance(
                    &TREASURY_ACCOUNT
                ),
                100
            );
            assert_eq!(
                <Runtime as pallet_elections_phragmen::Trait>::Currency::can_slash(
                    &TREASURY_ACCOUNT,
                    10
                ),
                true
            ...
            assert_eq!(
                <Runtime as pallet_elections_phragmen::Trait>::Currency::can_reserve(
                    &TREASURY_ACCOUNT,
                    5
                ),
                true
            ...
            // transfer
            assert_eq!(
                <Runtime as pallet_elections_phragmen::Trait>::Currency::free_balance(
                    &TREASURY_ACCOUNT
                ),
                95
            );
            ...
            assert_ok!(
                <Runtime as pallet_elections_phragmen::Trait>::Currency::transfer(
                    &TREASURY_ACCOUNT,
                    &ALICE,
                    11,
...
fn currency_adapter_transferring_too_high_value_should_not_panic() {
    ExtBuilder::default().build().execute_with(|| {
        TreasuryCurrencyAdapter::make_free_balance_be(
            &TREASURY_ACCOUNT,
            Balance::max_value().into(),
        );
...
fn multi_token_currency_extended_mint() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_ok!(<MultiTokenCurrencyAdapter<Runtime>>::mint(currency_id, &ALICE.into(), minted_amount));
...
fn multi_token_currency_extended_burn_and_settle_verify_burned_amount() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_ok!(
            <MultiTokenCurrencyAdapter<Runtime>>::burn_and_settle(currency_id, &ALICE.into(), 500_000)
        );
