use crate::mock::{new_tester, AccountId, Asset, MockRuntime, Origin, System, TestEvent};
...
fn mint_should_increase_balance_and_total_issuance() {
    new_tester().execute_with(|| {
        ...
        assert_eq!(
            Account::<MockRuntime>::get(&asset_id, &alice).free,
            500.into()
        );
...
fn mint_should_raise_event() {
    new_tester().execute_with(|| {
        ...
        assert_ok!(Asset::do_mint(asset_id, &alice, 500.into()));
        assert_eq!(
            TestEvent::generic_asset(RawEvent::Minted(asset_id, alice, 500.into())),
            last_event()
        );
    });
}
...
fn burn_should_raise_event() {
    new_tester().execute_with(|| {
        ...
        assert_ok!(Asset::do_burn(asset_id, &alice, 20.into()));
        assert_eq!(
            TestEvent::generic_asset(RawEvent::Burned(asset_id, alice, 20.into())),
            last_event()
        );
    });
}
...
fn transfer_free_balance() {
    new_tester().execute_with(|| {
        ...
        assert_eq!(
            Account::<MockRuntime>::get(&asset_id, &bob).free,
            750.into()
        );
