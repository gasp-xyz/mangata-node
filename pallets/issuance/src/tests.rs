use super::*;
use mock::{new_test_ext, Issuance, System, Test, Tokens, BlocksPerRound, roll_to_while_minting};
...
fn linear_issuance_works() {
	new_test_ext()
		.execute_with(|| {
            ...
            let session_issuance = <Issuance as GetIssuance>::get_all_issuance(session_number).expect("session issuance is always populated in advance");
            let block_issuance = (session_issuance.0 + session_issuance.1 + session_issuance.2 ) / (BlocksPerRound::get().saturated_into::<u128>());
...
fn linear_issuance_doesnt_change_upon_burn() {
	new_test_ext()
		.execute_with(|| {
            ...
            orml_tokens::MultiTokenCurrencyAdapter::<Test>::burn_and_settle(0u32.into(), &0u128, 100_000_000).unwrap();
...
fn issuance_does_not_stop_upon_burn() {
	new_test_ext()
		.execute_with(|| {
            ...
            orml_tokens::MultiTokenCurrencyAdapter::<Test>::burn_and_settle(0u32.into(), &0u128, 100_000).unwrap();
