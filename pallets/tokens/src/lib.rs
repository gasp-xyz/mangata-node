...
use frame_support::{
    ...
    traits::Get,
    traits::{
        BalanceStatus, ExistenceRequirement, Imbalance, LockIdentifier, SignedImbalance,
        WithdrawReasons,
    },
    weights::Weight,
...
pub use multi_token_currency::{
    MultiTokenCurrency, MultiTokenCurrencyExtended, MultiTokenLockableCurrency,
    ...
    T: Trait,
{
        ...
        if !remaining_slash.is_zero() {
            ...
            (
                Self::NegativeImbalance::new(
                    currency_id,
                    free_slashed_amount + reserved_slashed_amount,
                ),
                remaining_slash,
            )
        } else {
            (
                Self::NegativeImbalance::new(currency_id, value),
                remaining_slash,
            )
        }
    }
    ...
    fn make_free_balance_be(
        ...
        <Accounts<T>>::mutate(
            ...
            |account| -> Result<SignedImbalance<Self::Balance, Self::PositiveImbalance>, ()> {
                let imbalance = if account.free <= value {
                    SignedImbalance::Positive(MultiTokenPositiveImbalance::new(
                        currency_id,
                        value - account.free,
                    ))
                } else {
                    SignedImbalance::Negative(MultiTokenNegativeImbalance::new(
                        currency_id,
                        account.free - value,
                    ))
                };
        ...
        .unwrap_or_else(|_| {
            SignedImbalance::Positive(MultiTokenPositiveImbalance::zero(currency_id))
        })
    }
}
    ...
    T: Trait,
{
    fn create(address: &T::AccountId, amount: T::Balance) -> T::CurrencyId {
        ...
        let _ =
            <Self as MultiTokenCurrency<T::AccountId>>::deposit_creating(token_id, address, amount);
        token_id
    }
    ...
    fn mint(
        ...
        let current_balance =
            <Self as MultiTokenCurrency<T::AccountId>>::total_balance(currency_id, address);
        // check for overflow while minting
        current_balance
            .checked_add(&amount)
            .ok_or(Error::<T>::BalanceOverflow)?;

        let _ = <Self as MultiTokenCurrency<T::AccountId>>::deposit_creating(
            currency_id,
            address,
            amount,
        );
