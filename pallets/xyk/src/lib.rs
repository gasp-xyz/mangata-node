		...
		fn build(&self) {
			self.created_pools_for_staking.iter().for_each(|(account_id, native_token_id, native_token_amount, pooled_token_id, pooled_token_amount, liquidity_token_id)| {
                if <T as Config>::Currency::exists({*liquidity_token_id}.into()){
                    assert!(<Pallet<T>>::mint_liquidity( T::Origin::from(Some(account_id.clone()).into()), *native_token_id, *pooled_token_id, *native_token_amount, *pooled_token_amount).is_ok(), "Pool mint failed");
                }
                else{
                    ...
                    assert!(<Pallet<T>>::create_pool( T::Origin::from(Some(account_id.clone()).into()), *native_token_id, *native_token_amount, *pooled_token_id, *pooled_token_amount).is_ok(), "Pool creation failed");
                }
            })
		}
	}
...
impl<T: Config> XykFunctionsTrait<T::AccountId> for Pallet<T> {
    ...
    fn create_pool(
        ...
        let liquidity_asset_id: Self::CurrencyId =
            <T as Config>::Currency::create(&sender, initial_liquidity.into()).map_err(|_| Error::<T>::LiquidityTokenCreationFailed)?.into();
