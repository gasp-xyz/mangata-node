...
use sp_runtime::{
    generic::BlockId,
    traits::{AtLeast32Bit, Block as BlockT, Extrinsic, NumberFor, Zero},
};
...
type BoxedReadyIterator<Hash, Data> =
    Box<dyn Iterator<Item = Arc<sc_transaction_graph::base_pool::Transaction<Hash, Data>>> + Send>;

type ReadyIteratorFor<PoolApi> = BoxedReadyIterator<
    sc_transaction_graph::ExtrinsicHash<PoolApi>,
    sc_transaction_graph::ExtrinsicFor<PoolApi>,
>;
    ...
    PoolApi: ChainApi<Block = Block> + 'static,
{
    ...
    pub fn new_test(
        ...
        let pool = Arc::new(sc_transaction_graph::Pool::new(
            Default::default(),
            pool_api.clone(),
        ));
    ...
    PoolApi: 'static + ChainApi<Block = Block>,
{
    ...
    type InPoolTransaction =
        sc_transaction_graph::base_pool::Transaction<TxHash<Self>, TransactionFor<Self>>;
    ...
    fn submit_one(
        ...
        self.metrics
            .report(|metrics| metrics.submitted_transactions.inc());
    ...
    fn ready_at(&self, at: NumberFor<Self::Block>) -> PolledIterator<PoolApi> {
        ...
        Box::pin(self.ready_poll.lock().add(at).map(|received| {
    ...
    pub fn next_required(
        ...
        match *self {
            ...
            Self::Scheduled(revalidate_at_time, revalidate_at_block) => {
                let is_required = revalidate_at_time
                ...
                async move {
                    ...
                    if let Some(ref tree_route) = tree_route {
                        for retracted in tree_route.retracted() {
                            ...
                            pool.validated_pool()
                                .on_block_retracted(retracted.hash.clone());
                        }
                    ...
                    metrics.report(|metrics| {
                        metrics
                            .block_transactions_pruned
                            .inc_by(pruned_log.len() as u64)
                    });
                    ...
                    if let (true, Some(tree_route)) = (next_action.resubmit, tree_route) {
                        ...
                        for retracted in tree_route.retracted() {
                            ...
                            metrics.report(|metrics| {
                                metrics
                                    .block_transactions_resubmitted
                                    .inc_by(resubmitted_to_report)
                            });
                        }
                    ...
                    if next_action.revalidate {
                        let hashes = pool
                            ...
                            .collect();
                        revalidation_queue
                            .revalidate_later(block_number, hashes)
                            .await;
