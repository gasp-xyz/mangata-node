...
fn create_l1_update_with_offset(requests: Vec<L1UpdateRequest>, offset: sp_core::U256) -> eth_api::L1Update{
	...
	ExtBuilder::new()
		.execute_with_default_mocks(|| {
			...
			assert_event_emitted!(
				Event::PendingRequestStored((ALICE, H256::from(hex!("2a48bbdcd86e4e5571feef2579e2c4098c95b5aecc82a603c873429bf72651c3"))))
			);
		});
}
...
fn create_pending_update_after_dispute_period() {
	ExtBuilder::new()
		.execute_with_default_mocks(|| {
			let update1 = create_l1_update(vec![
			...
			let update2 = create_l1_update_with_offset(vec![
...
fn updates_to_remove_executed_after_dispute_period() {
	ExtBuilder::new()
		...
		.execute_with_default_mocks(|| {
			...
			let l2_updates_to_remove = create_l1_update_with_offset(vec![
				L1UpdateRequest::Remove(
					eth_api::L2UpdatesToRemove{
