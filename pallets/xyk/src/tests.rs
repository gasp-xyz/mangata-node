...
fn initialize() {
	...
	<pallet_assets::Module<Test>>::assets_issue(&accId, &amount);
	//XykStorage::create_asset(Origin::signed(2), 1000000);
...
fn create_pool_W() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(<pallet_assets::Module<Test>>::total_supply(2), 1000000); // total liquidity assets
		assert_eq!(<pallet_assets::Module<Test>>::balance(2, 2), 1000000); // amount of liquidity assets owned by user by creating pool / initial minting (500+500)
		assert_eq!(<pallet_assets::Module<Test>>::balance(0, 2), 500000); // amount in user acc after creating pool / initial minting
		assert_eq!(<pallet_assets::Module<Test>>::balance(1, 2), 500000); // amount in user acc after creating pool / initial minting
	});
}
...
// 		XykStorage::mint_liquidity(Origin::signed(2), 0, 1, 250000);
...
// 		XykStorage::mint_liquidity(Origin::signed(2), 1, 0, 250000);
...
// 		assert_err!(
// 			XykStorage::mint_liquidity(Origin::signed(2), 0, 1, 750000,),
// 			Error::<Test>::NotEnoughAssets,
// 		);
// 	});
// }
...
// 		assert_err!(
// 			XykStorage::mint_liquidity(Origin::signed(2), 1, 0, 750000,),
// 			Error::<Test>::NotEnoughAssets,
// 		);
// 	});
// }
...
// 		XykStorage::burn_liquidity(Origin::signed(2), 0, 1, 500000);
...
// 		initialize();
// 		// burning 250000 liquidity assetId2 of pool 0 1
// 		XykStorage::burn_liquidity(Origin::signed(2), 1, 0, 500000);
...
// 		assert_err!(
// 			XykStorage::burn_liquidity(Origin::signed(2), 0, 1, 1500000,),
// 			Error::<Test>::NotEnoughAssets,
// 		);
// 	});
// }
...
fn sell_W() {
	new_test_ext().execute_with(|| {
		...
		XykStorage::sell_asset(Origin::signed(2), 0, 1, 250000, 0);

		assert_eq!(<pallet_assets::Module<Test>>::balance(0, 2), 250000); // amount in user acc after selling
		assert_eq!(<pallet_assets::Module<Test>>::balance(1, 2), 666332); // amount in user acc after buying (check rounding should be 666333?)
		assert_eq!(XykStorage::asset_pool((0, 1)), 750000); // amount in pool map
		assert_eq!(XykStorage::asset_pool((1, 0)), 333668); // amount in pool map
		                                            // assert_eq!(XykStorage::get_free_balance(0, 1), 750000); // amount in vault acc
...
fn sell_W_other_way() {
	new_test_ext().execute_with(|| {
		initialize();
		// selling 250000 assetId 1 of pool 0 1
		XykStorage::sell_asset(Origin::signed(2), 1, 0, 250000, 0);

		assert_eq!(<pallet_assets::Module<Test>>::balance(0, 2), 666332); // amount in user acc after selling
		assert_eq!(<pallet_assets::Module<Test>>::balance(1, 2), 250000); // amount in user acc after buying (check rounding should be 666333?)
		assert_eq!(XykStorage::asset_pool((0, 1)), 333668); // amount in pool map
		assert_eq!(XykStorage::asset_pool((1, 0)), 750000); // amount in pool map
		                                            // assert_eq!(XykStorage::get_free_balance(0, 1), 750000); // amount in vault acc
		                                            // assert_eq!(XykStorage::get_free_balance(1, 1), 333668); // amount in vault acc
	});
}
...
fn buy_W_other_way() {
	new_test_ext().execute_with(|| {
		...
		XykStorage::buy_asset(Origin::signed(2), 1, 0, 166333, 500000);

		assert_eq!(<pallet_assets::Module<Test>>::balance(0, 2), 666333); // amount in user acc after selling
		assert_eq!(<pallet_assets::Module<Test>>::balance(1, 2), 249999); // amount in user acc after buying (check rounding should be 666333?)); // amount in user acc after buying
		assert_eq!(XykStorage::asset_pool((0, 1)), 333667); // amount in pool map
		assert_eq!(XykStorage::asset_pool((1, 0)), 750001); // amount in pool map
		                                            // assert_eq!(XykStorage::get_free_balance(0, 1), 750000); // amount in vault acc
		                                            // assert_eq!(XykStorage::get_free_balance(1, 1), 333668); // amount in vault acc
	});
}
...
fn buy_N_not_enough_reserve() {
	new_test_ext().execute_with(|| {
		...
		assert_err!(
			XykStorage::buy_asset(Origin::signed(2), 0, 1, 1500000, 5000000),
...
fn buy_N_not_enough_selling_assset() {
	new_test_ext().execute_with(|| {
		...
		assert_err!(
			XykStorage::buy_asset(Origin::signed(2), 0, 1, 150000, 5000000),
			Error::<Test>::NotEnoughAssets,
		);
	});
}
...
fn sell_N_not_enough_selling_assset() {
	new_test_ext().execute_with(|| {
		...
		assert_err!(
			XykStorage::sell_asset(Origin::signed(2), 0, 1, 750000, 0),
			Error::<Test>::NotEnoughAssets,
		);
	});
}
