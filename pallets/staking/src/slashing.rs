...
pub fn do_slash<T: Trait>(
	...
	let value = ledger.slash(value, T::Tokens::minimum_balance(stash_liquidity_token.into()).into());

	if !value.is_zero() {
		let (imbalance, missing) = T::Tokens::slash(stash_liquidity_token.into(), stash, value.into());
