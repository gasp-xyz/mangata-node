...
fn sudo_basics() {
    ...
    new_test_ext().execute_with(|| {
        ...
        let call = Box::new(Call::Logger(LoggerCall::privileged_i32_log(42, 1_000)));
        assert_noop!(
            SudoOrigin::sudo(Origin::signed(2), call),
...
fn sudo_unchecked_weight_basics() {
    new_test_ext().execute_with(|| {
        // A privileged function should work when `sudo` is passed the root `key` as origin.
        let call = Box::new(Call::Logger(LoggerCall::privileged_i32_log(42, 1_000)));
        assert_ok!(SudoOrigin::sudo_unchecked_weight(
            RawOrigin::Root.into(),
            call,
            1_000
        ));
        ...
        let call = Box::new(Call::Logger(LoggerCall::privileged_i32_log(42, 1)));
        let sudo_unchecked_weight_call = SudoOriginCall::sudo_unchecked_weight(call, 1_000);
...
fn sudo_as_basics() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            SudoOrigin::sudo_as(Origin::signed(3), 2, call),
            DispatchError::BadOrigin
        );
