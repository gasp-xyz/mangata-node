...
pub trait MultiCurrency<AccountId>: MangataPrimitives {
	...
	fn ensure_can_withdraw(currency_id: <Self as MangataPrimitives>::TokenId, who: &AccountId, amount: Self::Balance) -> DispatchResult;
	...
	/// total issuance.
	fn deposit(currency_id: <Self as MangataPrimitives>::TokenId, who: &AccountId, amount: Self::Balance) -> DispatchResult;

	/// Remove `amount` from the balance of `who` under `currency_id` and reduce
	/// total issuance.
	fn withdraw(currency_id: <Self as MangataPrimitives>::TokenId, who: &AccountId, amount: Self::Balance) -> DispatchResult;
	...
	fn slash(currency_id: <Self as MangataPrimitives>::TokenId, who: &AccountId, amount: Self::Balance) -> Self::Balance;
}
...
pub trait MultiCurrencyExtended<AccountId>: MultiCurrency<AccountId> {
	...
	fn update_balance(currency_id: <Self as MangataPrimitives>::TokenId, who: &AccountId, by_amount: Self::Amount) -> DispatchResult;
}
...
pub trait MultiLockableCurrency<AccountId>: MultiCurrency<AccountId> {
	...
	fn set_lock(lock_id: LockIdentifier, currency_id: <Self as MangataPrimitives>::TokenId, who: &AccountId, amount: Self::Balance);
	...
	/// - maximum `amount`
	fn extend_lock(lock_id: LockIdentifier, currency_id: <Self as MangataPrimitives>::TokenId, who: &AccountId, amount: Self::Balance);

	/// Remove an existing lock.
	fn remove_lock(lock_id: LockIdentifier, currency_id: <Self as MangataPrimitives>::TokenId, who: &AccountId);
}
...
pub trait MultiReservableCurrency<AccountId>: MultiCurrency<AccountId> {
	...
	/// be returned.
	fn slash_reserved(currency_id: <Self as MangataPrimitives>::TokenId, who: &AccountId, value: Self::Balance) -> Self::Balance;
...
pub trait OnReceived<AccountId, TokenId, Balance>: MangataPrimitives {
