...
fn buy_and_burn_sell_mangata() {
        new_test_ext().execute_with(|| {
            ...
            assert_eq!(XykStorage::balance(0, 2),7990000); // user acc - regular trade result
            assert_eq!(XykStorage::balance(1, 2),7009871); // user acc - regular trade result
            assert_eq!(XykStorage::balance(0, XykStorage::account_id()),2009995); // vault - pool (0-1), pool (0-2), treasury (5)
            assert_eq!(XykStorage::balance(1, XykStorage::account_id()),2990129); // vault - regular trade result
...
fn buy_and_burn_sell_other_for_mangata() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(XykStorage::balance(0, 2),8009871); // user acc - regular trade result
        assert_eq!(XykStorage::balance(1, 2),6990000); // user acc - regular trade result
        assert_eq!(XykStorage::balance(0, XykStorage::account_id()),1990125); // vault - pool (0-1), pool (0-2), treasury (4)
        assert_eq!(XykStorage::balance(1, XykStorage::account_id()),3010000); // vault - regular trade result
...
fn buy_and_burn_sell_only_sold_has_mangata_pair() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(XykStorage::balance(1, 2),6990000); // user acc - regular trade result
        assert_eq!(XykStorage::balance(3, 2),8009871); // user acc - regular trade result
        assert_eq!(XykStorage::balance(0, XykStorage::account_id()),1999996); // vault - pool (0-1), pool (0-2), treasury (4)
        assert_eq!(XykStorage::balance(1, XykStorage::account_id()),3010000); // vault - regular trade result
        assert_eq!(XykStorage::balance(3, XykStorage::account_id()),1990129); // vault - regular trade result
        assert_eq!(XykStorage::treasury(0), 4); // 4 mangata in treasury
        assert_eq!(XykStorage::treasury(1), 0);         
        assert_eq!(XykStorage::treasury_burn(0), 0);
        assert_eq!(XykStorage::treasury_burn(1), 0);
...
fn buy_and_burn_sell_only_bought_has_mangata_pair() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(XykStorage::balance(1, 2),7009871); // user acc - regular trade result
        assert_eq!(XykStorage::balance(3, 2),7990000); // user acc - regular trade result
        assert_eq!(XykStorage::balance(0, XykStorage::account_id()),1999997); // vault - pool (0-1), pool (0-2), treasury (4)
        assert_eq!(XykStorage::balance(1, XykStorage::account_id()),2990129); // vault - regular trade result
        assert_eq!(XykStorage::balance(3, XykStorage::account_id()),2010000); // vault - regular trade result
        assert_eq!(XykStorage::treasury(0), 3); // 3 mangata in treasury
        assert_eq!(XykStorage::treasury(1), 0);         
        assert_eq!(XykStorage::treasury_burn(0), 0);
        assert_eq!(XykStorage::treasury_burn(1), 0);
});
...
fn buy_and_burn_sell_both_have_mangata_pair() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(XykStorage::balance(1, 2),7009871); // user acc - regular trade result
        assert_eq!(XykStorage::balance(2, 2),7990000); // user acc - regular trade result
        assert_eq!(XykStorage::balance(0, XykStorage::account_id()),1999997); // vault - pool (0-1), pool (0-2), treasury (4)
        assert_eq!(XykStorage::balance(1, XykStorage::account_id()),2990129); // vault - regular trade result
        assert_eq!(XykStorage::balance(2, XykStorage::account_id()),2010000); // vault - regular trade result
...
fn buy_and_burn_sell_none_have_mangata_pair() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(XykStorage::balance(3, 2),7990000); // user acc - regular trade result
        assert_eq!(XykStorage::balance(4, 2),9009871); // user acc - regular trade result
        assert_eq!(XykStorage::balance(0, XykStorage::account_id()),2000000); // vault - pool (0-1), pool (0-2), treasury (4)
        assert_eq!(XykStorage::balance(3, XykStorage::account_id()),2010000); // vault - regular trade result
        assert_eq!(XykStorage::balance(4, XykStorage::account_id()),990129); // vault - regular trade result
...
//         assert_eq!(XykStorage::asset_pool((0, 1)), 9992491552997121764); // amount in pool map
...
//         initialize();
//         assert_err!(
//             XykStorage::mint_liquidity(Origin::signed(2), 1, 0, 0,),
//             Error::<Test>::ZeroAmount,
