...
use substrate_prometheus_endpoint::{SourcedGauge, SourcedCounter, MetricSource, Opts, PrometheusError, Registry};

pub fn register(registry: Option<&Registry>, client: Arc<crate::service::FullClient>) -> Result<(), PrometheusError> {
...
where C: ProvideRuntimeApi<Block>,
C: Send + Sync + 'static,
C: HeaderBackend<Block>,
		C::Api: RolldownRuntimeApi<Block, pallet_rolldown::messages::L1Update, pallet_rolldown::messages::L1> {
	...
	pub fn register(registry: &Registry, client: Arc<C>) -> Result<(), PrometheusError> {
		substrate_prometheus_endpoint::register(
			SourcedCounter::new(
				&Opts::new("substrate_rolldown_last_processed_request_on_l2", "Last Processed Request On L2").variable_label("for_L1"),
...
C: HeaderBackend<Block>,
	...
	fn collect(&self, mut set: impl FnMut(&[&str], Self::N)) {
		...
		set(&["Ethereum"], self.0.runtime_api().get_last_processed_request_on_l2(at, pallet_rolldown::messages::L1::Ethereum).unwrap_or(None).unwrap_or_default().saturated_into::<u64>());
	}
}
...
C: HeaderBackend<Block>,
	...
	pub fn register(registry: &Registry, client: Arc<C>) -> Result<(), PrometheusError> {
		substrate_prometheus_endpoint::register(
			SourcedCounter::new(
				&Opts::new("substrate_rolldown_number_of_pending_requests", "Number Of Pending Requests").variable_label("for_L1"),
...
C: HeaderBackend<Block>,
	...
	fn collect(&self, mut set: impl FnMut(&[&str], Self::N)) {
		let at = self.0.info().best_hash;
		set(&["Ethereum"], self.0.runtime_api().get_number_of_pending_requests(at, pallet_rolldown::messages::L1::Ethereum).unwrap_or(None).unwrap_or_default().saturated_into::<u64>());
	}
}
...
,
	...
	fn collect(&self, mut set: impl FnMut(&[&str], Self::N)) {
		let at = self.0.info().best_hash;
		set(&["Total Deposits"], self.0.runtime_api().get_total_number_of_deposits(at).unwrap_or_default().saturated_into::<u64>());
		set(&["Total Withdrawals"], self.0.runtime_api().get_total_number_of_withdrawals(at).unwrap_or_default().saturated_into::<u64>());
		set(&["Total Swaps"], self.0.runtime_api().get_total_number_of_swaps(at).unwrap_or_default().saturated_into::<u64>());
	}
}