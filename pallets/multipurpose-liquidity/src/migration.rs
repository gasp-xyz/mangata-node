...
-> Result<(), &'static str>{
    ...
    // represents reserves
    // xyk, staking, total
    let mut user_reserve_info: BTreeMap<(T::AccountId, TokenId), (Balance, Balance, Balance)> = BTreeMap::new();

    for (collator_account, collator_info) in collator_storage.iter(){
        if let Some((xyk_reserve, staking_reserve, total_reserve)) = user_reserve_info.get_mut(&(collator_account.clone(), collator_info.liquidity_token)) {
            ...
            *total_reserve = total_reserve.saturating_add(collator_info.bond);
            ...
            user_reserve_info.insert((collator_account.clone(), collator_info.liquidity_token),
    ...
    for (delegator_account, delegator_info) in delegator_storage.iter(){
        for delegation in delegator_info.delegations.0.iter(){
            if let Some((xyk_reserve, staking_reserve, total_reserve)) = user_reserve_info.get_mut(&(delegator_account.clone(), delegation.liquidity_token)) {
                ...
                *total_reserve = total_reserve.saturating_add(delegation.amount);
                ...
                user_reserve_info.insert((delegator_account.clone(), delegation.liquidity_token),
    ...
    for ( (account, liquidity_token), amount) in activation_storage.iter(){
        if let Some((xyk_reserve, staking_reserve, total_reserve)) = user_reserve_info.get_mut(&(account.clone(), *liquidity_token)) {
            *xyk_reserve = xyk_reserve.saturating_add(*amount);
            *total_reserve = total_reserve.saturating_add(*amount);
        } else {
            user_reserve_info.insert((account.clone(), *liquidity_token),
                (*amount, Balance::zero(), *amount));
        };
    }

    for ((account, liquidity_token), (xyk_reserve, staking_reserve, total_reserve)) in user_reserve_info.iter(){
        assert_eq!(xyk_reserve.saturating_add(*staking_reserve), *total_reserve);
        assert_eq!(T::Tokens::reserved_balance((*liquidity_token).into(), account).into(), *total_reserve);
...
-> Result<(), &'static str>{
    ...
    let user_reserve_info: BTreeMap<(T::AccountId, TokenId), (Balance, Balance, Balance)> = Pallet::<T>::get_temp_storage("user_reserve_info").expect("temp storage was set");
