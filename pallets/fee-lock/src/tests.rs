...
fn update_fee_lock_metadata_works() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			TokenTimeout::update_fee_lock_metadata(RuntimeOrigin::root(), Some(500), Some(0), Some(1000), None,),
			Error::<Test>::InvalidTimeoutMetadata
		);

		assert_noop!(
			TokenTimeout::update_fee_lock_metadata(RuntimeOrigin::root(), Some(500), None, Some(1000), None,),
			Error::<Test>::InvalidTimeoutMetadata
		);

		assert_noop!(
			TokenTimeout::update_fee_lock_metadata(RuntimeOrigin::root(), Some(0), Some(500), Some(1000), None,),
			Error::<Test>::InvalidTimeoutMetadata
		);

		assert_noop!(
			TokenTimeout::update_fee_lock_metadata(RuntimeOrigin::root(), None, Some(500), Some(1000), None,),
			Error::<Test>::InvalidTimeoutMetadata
		);

		assert_ok!(TokenTimeout::update_fee_lock_metadata(
			RuntimeOrigin::root(),
			Some(1000),
			Some(500),
			Some(1000),
		...
		assert_eq!(
			...
			Some(TimeoutMetadataInfo {
				...
				whitelisted_tokens: {
					BoundedBTreeSet::<TokenId, Balance, MaxCuratedTokens>::try_from(
						vec![1, 2]
							.into_iter()
							.collect::<BTreeSet<TokenId>>(),
					)
		...
		assert_noop!(
			TokenTimeout::update_fee_lock_metadata(RuntimeOrigin::root(), None, Some(0), None, None,),
			Error::<Test>::InvalidTimeoutMetadata
		);

		assert_noop!(
			TokenTimeout::update_fee_lock_metadata(RuntimeOrigin::root(), Some(0), None, None, None,),
			Error::<Test>::InvalidTimeoutMetadata
		);
